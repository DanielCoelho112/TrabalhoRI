function [theta1_deg,theta2_deg,theta3_deg,theta4_deg,theta5_deg,theta6_deg]=Cart2Joint(r,red_theta2,red_theta3,red_theta5)
%   Função para calcular o espaço de juntas sabendo o espaço cartesiano

%Comprimentos dos elos do manipulador

[La, Lb, Lc, Ld, Le, Lf] = deal(0, 150, 360, 100, 430, 100);
...
C=(red_theta2*sqrt(PWx^2+PWy^2)-Lb)^2+PWz^2-Lc^2-Ld^2-Le^2;
...
%Matriz global, desde a base até ao end effector
o_T_e=trans3(r(1),r(2),r(3))*rotz(deg2rad(r(6)))*roty(deg2rad(r(5)))*rotx(deg2rad(r(4)));

%Denavit apenas dos 5 primeiros elos
...
%   Sequência de transformações desde a origem (O) até ao gripper (G)
   ...
   %Matriz de transformação  dos ultimos 3 elos, é onde reside o
   %a1,a2,a3,s3,n3
w_T_e=inv(OTG)*o_T_e;
